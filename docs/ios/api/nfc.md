...
//00B0850000 读取05文件   01000100000000000000000000000000000000000000003231303836363733323900000000000000000000000000000000000000000000
//00B201C400 查询第一次交易  000E 000000 00000064 02 101020203040 20211116 202522
//805c000204 查询余额  00000578  4个字节
```

* 选择主文件  `00a40000023f00`(住建部卡主文件一般都是这条指令)

------

* 读取05文件 `00b0850000`,指令响应报文格式:
...
09-16 | 应用序列号 | 8 |   | 2：城市代码BCD<br>1：卡主类型BCD<br>1：卡子类型BCD<br>4：发行流水HEX
17 | 卡主类型 | 1 | BCD |  
18 | 卡子类型 | 1 | BCD |  
...
* 选择文件标识 `00a40000023f01` 、`00a4000002df01` 、`00a4000002df04` 、
`00a4040009a00000000386980701 `等等, 根据不同的城市，所处的文件可能有所不同

------

* 读取15文件 `00b0950000`,响应报文如下:
...
13-20 | 应用序列号 | 8 | BCD | 2：城市代码BCD<br>1：卡主类型BCD<br>1：卡子类型BCD<br>4：发行流水HEX
...
* 电子钱包交易记录 `00b2xxc400（18号文件）`、 `00b2xxd400（1a号文件）`,根据不同城市不同的卡类型去读取,`xx`标识读取的第几条记录:
...
举个例子：`0020000003123456` 回包`9000`即为成功
...
举个例子：`805000020B0100000001112233445566`
